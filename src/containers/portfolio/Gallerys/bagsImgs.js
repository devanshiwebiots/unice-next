  ...
  return (
    <Fragment>
      {BagsImgData.length > 0
        ...
        : ""}
      {photoIndex.isOpen && <Lightbox mainSrc={`/assets/images${photoIndex?.image}`} nextSrc={`/assets/images${BagsImgData[(photoIndex.index + 1) % BagsImgData.length].img}`} prevSrc={`/assets/images${BagsImgData[(photoIndex.index + BagsImgData.length - 1) % BagsImgData.length].img}`} imageTitle={photoIndex.index + 1 + "/" + BagsImgData.length} onCloseRequest={() => setPhotoIndex({ ...photoIndex, isOpen: false })} onMovePrevRequest={onMovePrev} onMoveNextRequest={onMoveNext} />}
    </Fragment>
  );
};

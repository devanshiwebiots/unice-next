  ...
  return (
    <Fragment>
      {ShoesImgData.length > 0
        ...
        : ""}
      {photoIndex.isOpen && <Lightbox mainSrc={`/assets/images${photoIndex?.image}`} nextSrc={`/assets/images${ShoesImgData[(photoIndex.index + 1) % ShoesImgData.length].img}`} prevSrc={`/assets/images${ShoesImgData[(photoIndex.index + ShoesImgData.length - 1) % ShoesImgData.length].img}`} imageTitle={photoIndex.index + 1 + "/" + ShoesImgData.length} onCloseRequest={() => setPhotoIndex({ ...photoIndex, isOpen: false })} onMovePrevRequest={onMovePrev} onMoveNextRequest={onMoveNext} />}
    </Fragment>
  );
};

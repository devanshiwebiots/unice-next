    ...
    <Fragment>
      ...
      {photoIndex.isOpen && (
        <>
          ...
          <Lightbox mainSrc={`/assets/images${photoIndex.image}`} nextSrc={`/assets/images${AllImgData[(photoIndex.index + 1) % AllImgData.length].img}`} prevSrc={`/assets/images${AllImgData[(photoIndex.index + AllImgData.length - 1) % AllImgData.length].img}`} imageTitle={photoIndex.index + 1 + "/" + AllImgData.length} onCloseRequest={() => setPhotoIndex({ ...photoIndex, isOpen: false })} onMovePrevRequest={onMovePrev} onMoveNextRequest={onMoveNext} />
        </>
      )}
    </Fragment>
  );
